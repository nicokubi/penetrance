...
vignette: >
  %\VignetteIndexEntry{Penetrance Estimation with PenEstim}
...
The R package `PenEstim` provides an estimation of age-specific penetrance for complex 
...
using the PenEstim package based on family data and user-specified inputs. 
...
need to be specified. PenEstim provides the user with a flexible approach to prior 
...
The version of the algorithm show as an example below includes:
...
The baseline_data must be a data frame with baseline data for females and males. 
```{r}
...
in the `prior_params_test` object (see above).
...
output <- out_sim_COL <- PenEstim(
    pedigree  = dat, twins = NULL, n_chains = 1, n_iter_per_chain = 10000, 
    ncores = 4, baseline_data = baseline_data_default, af  = 0.0001, 
    prior_params = prior_params_test, burn_in = 0.1, median_max = TRUE,  
    ageImputation = TRUE
)
```
